...
namespace WebApplication.Processing
{
    public class UpdateParameters : ForgeAppBase
    {
        ...
        public override List<string> ActivityCommandLine =>
            new List<string>
            {
                $"$(appbundles[0].engine.path)\\InventorCoreConsole.exe /al \"$(appbundles[{ActivityId}].path)\" /ilod \"$(args[{InputDocParameterName}].path)\" /paramFile \"$(args[{InventorParameters}].path)\" /p"
            };
